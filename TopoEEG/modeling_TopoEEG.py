...
class TopoEEG(nn.Module):
    ...
    def _build_feature_extractors(self):
        for name, shape in self.input_shapes.items():
            if len(shape) == 1:  # 1D features 
                self.feature_extractors[name] = self._build_CNN_1D(shape)
            elif len(shape) == 2:  # 2D features 
                self.feature_extractors[name] = self._build_CNN_2D(shape)
    ...
    def _build_feature_level_classifier(self):
        ...
        for name, shape in self.input_shapes.items():
            if len(shape) == 1:
                ...
                total_dim += 32 * shape[1] * shape[2]  # 32 filters * n_nodes * n_nodes

        self.final_classifier = nn.Sequential(self.classifier_base(total_dim, self.classifier_base_n_out),
    ...
    def _build_score_level_classifier(self):
        ...
        for name, shape in self.input_shapes.items():
            if len(self.input_shapes[name]) == 1: 
                input_dim = 32 * self.input_shapes[name][0]
            elif len(shape) == 2:
                input_dim = 32 * self.input_shapes[name][1] * self.input_shapes[name][2]
            self.dense_layers[name] = self.classifier_base(input_dim, self.classifier_base_n_out)
